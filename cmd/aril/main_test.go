...
func projectRoot(t *testing.T) string {
	...
	// cmd/aril/main_test.go → project root.
...
// arilBinary builds (once per test process) a real aril binary so
...
func arilBinary(t *testing.T) string {
	t.Helper()
	buildOnce.Do(func() {
		root := projectRoot(t)
		dir, err := os.MkdirTemp("", "aril-test-bin-*")
		if err != nil {
			buildErr = err
			return
		}
		bin := filepath.Join(dir, "aril-test")
		cmd := exec.Command("go", "build", "-o", bin, "./cmd/aril")
		cmd.Dir = root
	...
	if buildErr != nil {
		t.Fatalf("build aril failed: %v", buildErr)
	}
	return buildBin
}

// runAril invokes the test-built aril binary from the project
// root and returns (stdout, stderr, exitCode).
func runAril(t *testing.T, args ...string) (string, string, int) {
	t.Helper()
	bin := arilBinary(t)
	...
	if ee, ok := err.(*exec.ExitError); ok {
		...
		t.Fatalf("aril failed unexpectedly: %v\n%s", err, stderr.String())
	}
	return stdout.String(), stderr.String(), exit
}

func TestHelloEndToEnd(t *testing.T) {
	stdout, stderr, exit := runAril(t, "run", "examples/core-language/hello/hello.aril")
	if exit != 0 {
		t.Fatalf("hello.aril exited %d (stderr: %s)", exit, stderr)
	}
	if stdout != "Aril is rising.\n" {
		t.Errorf("hello.aril stdout = %q; want \"Aril is rising.\\n\"", stdout)
	}
}

func TestFizzBuzzEndToEnd(t *testing.T) {
	stdout, _, exit := runAril(t, "run", "examples/core-language/fizzbuzz/fizzbuzz.aril")
	if exit != 0 {
		t.Fatalf("fizzbuzz.aril exited %d", exit)
	}
	lines := strings.Split(strings.TrimRight(stdout, "\n"), "\n")
	if len(lines) != 100 {
		t.Errorf("fizzbuzz.aril produced %d lines; want 100", len(lines))
	}
	...
	for _, c := range []struct {
		...
		if lines[c.idx-1] != c.want {
			t.Errorf("fizzbuzz.aril line %d = %q; want %q", c.idx, lines[c.idx-1], c.want)
		}
	}
}
...
func TestRuntimeModeEquivalence(t *testing.T) {
	examples := []string{
		...
		"examples/core-language/slice_toolkit/slice_toolkit.aril",           // slices.reverse → arilrt.SlicesReverse
		"examples/core-language/leaderboard/leaderboard.aril",               // sort.sortedBy + slices.dedup → arilrt helpers
...
func TestVersion(t *testing.T) {
	...
	if exit != 0 {
		t.Fatalf("aril version exited %d", exit)
	}
	if !strings.HasPrefix(stdout, "aril ") {
		t.Errorf("aril version stdout = %q; want 'aril ' prefix", stdout)
	}
}

func TestUnknownSubcommand(t *testing.T) {
	_, stderr, exit := runAril(t, "frobnicate")
	if exit != 2 {
...
func TestRunMissingArg(t *testing.T) {
	_, stderr, exit := runAril(t, "run")
	if exit != 2 {
		t.Errorf("aril run (no args) exit = %d; want 2", exit)
	}
	if !strings.Contains(stderr, "expected exactly one <file.aril>") {
		t.Errorf("expected usage hint in stderr; got %q", stderr)
	}
}

func TestEmitHello(t *testing.T) {
	stdout, _, exit := runAril(t, "emit", "examples/core-language/hello/hello.aril")
	if exit != 0 {
		t.Fatalf("aril emit hello.aril exit = %d", exit)
	}
	// Spot-check: the lowered Go contains package main, fmt
	// import, the Println call, and at least one //line
	// directive pointing back at the .aril source.
	for _, want := range []string{
		"package main",
		`import "fmt"`,
		`fmt.Println("Aril is rising.")`,
		"//line examples/core-language/hello/hello.aril:",
	} {
...
func TestEmitMissingArg(t *testing.T) {
	_, stderr, exit := runAril(t, "emit")
	if exit != 2 {
		t.Errorf("aril emit (no args) exit = %d; want 2", exit)
	}
...
// runArilStdin invokes the test-built aril binary with the
// given args, piping the supplied stdin string. Used by REPL
// tests that drive the interactive prompt non-interactively.
func runArilStdin(t *testing.T, stdin string, args ...string) (string, string, int) {
	t.Helper()
	bin := arilBinary(t)
	cmd := exec.Command(bin, args...)
	cmd.Dir = projectRoot(t)
...
func TestReplPrintsHello(t *testing.T) {
	...
	stdout, stderr, exit := runArilStdin(t, input, "repl")
	if exit != 0 {
...
func TestReplRejectsTopLevelControlFlow(t *testing.T) {
	input := "if true { 1 }\n:quit\n"
	_, stderr, exit := runArilStdin(t, input, "repl")
	if exit != 0 {
...
func TestReplRollsBackCompileFailure(t *testing.T) {
	...
	input := "import fmt\nlet x = oh dear\nlet y = 99\nfmt.println(y)\n:quit\n"
	stdout, _, exit := runArilStdin(t, input, "repl")
	if exit != 0 {
		t.Fatalf("repl exit = %d", exit)
	}
...
func TestReplAutoPrintLastOnly(t *testing.T) {
	...
	for _, want := range []string{"aril> 3\n", "aril> 7\n", "aril> 11\n"} {
...
func TestBuildOutputFlag(t *testing.T) {
	...
	_, stderr, exit := runAril(t, "build", "-o", outPath, "examples/core-language/hello/hello.aril")
	if exit != 0 {
		t.Fatalf("aril build -o failed: %d (stderr: %s)", exit, stderr)
	}

...
// Driver coverage for directory packages — every `.aril` file in a
...
// TestCrossPackageManifestRun — a project with a aril.toml: the entry
// file imports a user package (`myproj/utils`), whose directory's .aril
...
func TestCrossPackageManifestRun(t *testing.T) {
	...
	writeFile(t, dir, "main.aril", "import fmt\nimport myproj/utils\nfunc main() { fmt.println(shout(\"x\")) }\n")
	writeFile(t, filepath.Join(dir, "utils"), "strs.aril", "import strings\nfunc shout(s: string): string { return strings.toUpper(s) }\n")
...
func TestUnknownImportPath(t *testing.T) {
	dir := t.TempDir()
	writeFile(t, dir, "aril.toml", "[project]\nname = \"p\"\n")
	writeFile(t, dir, "main.aril", "import totally/unknown\nfunc main() {}\n")
	_, stderr, exit := runAril(t, "run", dir)
	if exit == 0 {
...
func TestCyclicPackageImport(t *testing.T) {
	...
	writeFile(t, dir, "aril.toml", "[project]\nname = \"c\"\n")
	writeFile(t, dir, "main.aril", "import c/a\nfunc main() {}\n")
	writeFile(t, filepath.Join(dir, "a"), "a.aril", "import c/b\nfunc fa() {}\n")
	writeFile(t, filepath.Join(dir, "b"), "b.aril", "import c/a\nfunc fb() {}\n")
	_, stderr, exit := runAril(t, "run", dir)
	if exit == 0 {
...
func TestDiamondImportNotACycle(t *testing.T) {
	...
	writeFile(t, dir, "aril.toml", "[project]\nname = \"d\"\n")
	writeFile(t, dir, "main.aril", "import d/a\nimport d/b\nimport fmt\nfunc main() { fmt.println(fc()) }\n")
	writeFile(t, filepath.Join(dir, "a"), "a.aril", "import d/c\nfunc fa(): int { return fc() }\n")
	writeFile(t, filepath.Join(dir, "b"), "b.aril", "import d/c\nfunc fb(): int { return fc() }\n")
	writeFile(t, filepath.Join(dir, "c"), "c.aril", "func fc(): int { return 42 }\n")
	stdout, stderr, exit := runAril(t, "run", dir)
	if exit != 0 {
...
func TestPackageMultiFileRun(t *testing.T) {
	dir := writePkg(t, map[string]string{
		"util.aril":  "import strings\nfunc shout(s: string): string { return strings.toUpper(s) }\n",
		"greet.aril": "func greeting(): string { return shout(\"hi\") }\n",
		"main.aril":  "import fmt\nfunc main() { fmt.println(greeting()) }\n",
	})
	stdout, stderr, exit := runAril(t, "run", dir)
	if exit != 0 {
...
// its absence is a Aril-coordinate error, not a leaked Go-toolchain one.
func TestPackageNoMain(t *testing.T) {
	dir := writePkg(t, map[string]string{
		"lib.aril": "func helper(): int { return 1 }\n",
	})
	_, stderr, exit := runAril(t, "run", dir)
	if exit == 0 {
...
func TestPackageCrossFileDuplicate(t *testing.T) {
	dir := writePkg(t, map[string]string{
		"a.aril": "func dup(): int { return 1 }\nfunc main() {}\n",
		"b.aril": "func dup(): int { return 2 }\n",
	})
	_, stderr, exit := runAril(t, "run", dir)
	if exit == 0 {
		t.Fatal("expected non-zero exit for a cross-file duplicate decl")
	}
	if !strings.Contains(stderr, "E0113") || !strings.Contains(stderr, "b.aril") {
		t.Errorf("stderr = %q; want E0113 attributed to b.aril", stderr)
	}
}
...
func TestPackageCrossFileDiagPath(t *testing.T) {
	dir := writePkg(t, map[string]string{
		"a.aril": "func main() {}\n",
		"b.aril": "func bad(): int { return \"no\" }\n",
	})
	_, stderr, exit := runAril(t, "run", dir)
	if exit == 0 {
		t.Fatal("expected non-zero exit for a type error")
	}
	if !strings.Contains(stderr, "b.aril") {
		t.Errorf("stderr = %q; want the diagnostic attributed to b.aril", stderr)
	}
}

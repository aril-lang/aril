...
// cmdRepl implements `aril repl` per RFC-0003. PR-REPL-1 lands
...
func cmdRepl(args []string) int {
	fs := flag.NewFlagSet("aril repl", flag.ContinueOnError)
	fs.SetOutput(os.Stderr)
	fs.Usage = func() {
		fmt.Fprintln(os.Stderr, "usage: aril repl")
		fs.PrintDefaults()
	}
...
func runRepl(stdin io.Reader, stdout, stderr io.Writer) int {
	...
	for {
		...
		if buf.Len() == 0 && strings.HasPrefix(strings.TrimSpace(line), ":") {
			...
			// where Aril's `:` punctuator (type annotations,
...
// renderStmt produces the body-position Aril line for a stmt
...
// render builds the synthetic Aril source for the current
// session. The output is a complete `.aril` file that compiles
...
// the original Aril source the user typed, with imports and
...
// importSilencesFor returns a Aril expression per imported
...
func importSilencesFor(imports []string) []string {
	var out []string
	for _, imp := range imports {
		// Aril grammar (grammar.ebnf §Import) is just
...
func runSession(s *replSession, stdout, stderr io.Writer) error {
	src, err := compileSourceToTempGo(s.render(), "repl.aril")
	if err != nil {
		return err
	}
...
func oneShot(s *replSession, kind, expr string, stdout, stderr io.Writer) {
	...
	src, err := compileSourceToTempGo(s.renderWith([]string{stmt}), "repl.aril")
	if err != nil {
		fmt.Fprintln(stderr, "repl:", err)
		return
	}

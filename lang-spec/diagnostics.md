...
| E0214 | E | Type has no member `name` | `type-system.md` T-Field, T-Method (member access over a Named receiver) | Access only a declared field or method of the receiver's type — a user class / interface / record, an opaque `extern` handle, or a bound stdlib value-handle. A bare type parameter has no known member set, so it is not diagnosed here. |

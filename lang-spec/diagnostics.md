...
| E0124 | E | Duplicate binding of a Go module | `manifest.md` §Resolution | Two dependencies bind the **same** Go module across the build graph (two `kind = "go"` tables, or a table and a `binding` package). At most one binding table may bind a given Go module — two would emit duplicate `extern` declarations of the same Go symbols into the one lowered module (the Cargo `links` invariant). Drop one binding, or consume the module through a single table (RFC-0010). |

...
| E0112 | E | Unexpected token | `grammar.ebnf` syntactic part | The parser was looking for a different shape; check the surrounding construct. The message names the expected and found tokens in their **source spelling** — a required literal backticked (`` expected `)` ``, `` expected `in` ``), a general position as a token *class* (`expected a type`, `expected an identifier`), and the found token as its backticked lexeme or a phrase for the lexeme-less classes (`a line break`, `end of file`) — never the internal lexer kind name (`Punct`, `Keyword`). When the unexpected token is a newline mid-expression, a trailing binary operator has left the expression without a right operand — a newline outside brackets ends the expression, so wrap the whole expression in parentheses `(...)` to continue it across lines. A leading `else` or postfix `catch` on a fresh line is the same rule seen from the other side (D11: a newline ends the construct) — the message names it directly (``` `else` must be on the same line as the closing `}` ```), since `} else` / `expr catch` must cuddle. |
...
| E0121 | E | External dependency not resolvable | `manifest.md` §Resolution | An import names a declared `[dep.<name>]` external module that cannot be resolved: its module is not present on disk (not yet fetched — run `aril get`, or point `replace` at a local copy), a `kind = "aril"`/`"binding"` module has no `aril.toml`, or a `kind = "go"` dependency's consumer-owned `path` binding table is absent. Distinct from E0117 (an *undeclared* import path). |
...
| E0124 | E | Duplicate binding of a Go module | `manifest.md` §Resolution | Two dependencies bind the **same** Go module across the build graph — any mix of `kind = "go"` tables and `kind = "binding"` packages. At most one binding may bind a given Go module — two would emit duplicate `extern` declarations of the same Go symbols into the one lowered module (the Cargo `links` invariant). Drop one binding, or consume the module through a single table (RFC-0010). |
| E0125 | W | **reserved** (implicit `binds-go` drift) | `manifest.md` §Compatibility axes | Reserved for a `kind = "binding"` warning: Go-level max-of-floors resolved a bound Go module **above** a binding package's self-declared `binds-go` floor, so the binding runs against a newer (un-tested-against) module version — implicit ABI drift. Unallocated until published binding packages land. |
| E0126 | E | Binding table names an absent Go symbol | `manifest.md` §Resolution / `ffi.md` §The generator | A `kind = "go"` table's `extern … @go("mod/pkg.Sym")` names a symbol the **pinned** Go module does not export (renamed or removed upstream). Validated at `aril get` time against the freshly-fetched module, so table drift is a loud Aril-coordinate diagnostic — not a silently-shrunk surface or a raw `go build` miss. Re-run `aril import` and update the table. |
| E0127 | E | Map/Set key type parameter is not comparable | `grammar.ebnf` §Constraint / `type-system.md` §Generic constraints | A declaration's own type parameter is used as a `Map`/`Set` **key** (`var index: Map<K, V>`) without a `Comparable`/`Ordered` bound. Go's map/set key type must satisfy `comparable`, and Aril mirrors Go's *explicit* constraints (never inferred) — so declare the bound: `K: Comparable`. The D10-clean stand-in for Go's raw `K does not satisfy comparable`. Fires on key positions reachable from the declaration's field types; a Map/Set constructed on a bare parameter only inside a method body is out of v1 scope. |
...
| E0214 | E | Type has no member `name` | `type-system.md` T-Field, T-Call (member access / method call over a receiver with a fully-known member set) | Access only a declared field or method of the receiver's type — a user class / interface / record, an opaque `extern` handle, a bound stdlib value-handle, a builtin-generic type (`Map`/`Set`/`Stack`/`List`/`[]T`/`Option`/`Result`/channel, whose method set is fixed), or a closed-method-set primitive (numeric / `bool`: none; `string`: `len`/`bytes`/`runes`; `error`: `error()`). A bare type parameter, an `Any`/`Dynamic`, or an `Unknown`-typed receiver has no known member set, so it is not diagnosed here. |
| E0215 | E | **reserved** (was: discarded slice `push`) | — | Retired with slice `.push` (D55): `[]T` is a value view with no `push`, so there is no discarded-functional-append to diagnose. A leftover `xs.push(e)` on a slice now fires a tailored **E0214** naming `List<T>` (the growable reference container). Code slot kept reserved to avoid renumbering. |
...
| E0217 | E | Module has no bound member `name` | `type-system.md` T-Field (member access over a bound stdlib namespace) | The name is a shipped stdlib module (`import strings`) but the member is not bound by Aril's binding surface — check the spelling against `binding-surface.md`, or the member is simply unbound in this release. Gated on the resolved namespace symbol, so a value or `extern` binding that shares the name is not affected. The miss carries a **tailored hint** naming the Aril binding (mirrors the E0214 slice→`List<T>` hint — teach the trap, not just the miss): a **case-variant "did you mean"** when the member differs from a bound one only by first-letter case (Aril lowercases Go's exported names, so a Go-PascalCase reach like `strings.Fields`/`math.Sqrt` is recovered to `strings.fields`/`math.sqrt`); otherwise a namespace-specific idiom hint for a *rename* (`sort.*` — a `sort.Slice`/`sort.by` reach — names `sort.sorted(xs, less)` / `sort.sortedBy(xs, key)`). |
| E0218 | E | Invalid stdlib-handle construction | `lowering-go.md` §Brace literals / `binding-surface.md` §handles | A bound stdlib handle cannot be brace-built freely: a *fieldless constructable* one (`sync.Mutex`) takes no fields — write `sync.Mutex{}`; a *constructable handle with init fields* (`http.Server`) accepts only its declared fields — an unknown field (`http.Server{ bogus: … }`) is rejected; an *obtain-only* one (`regexp.Regexp`) is returned from a constructor (`regexp.mustCompile(…)`), never a `{}` literal. |
...
| E0220 | E | Omitted field has no safe default | `lowering-go.md` §Container defaulting | A record/class literal may omit a field only when its zero value is safe — scalars (`0`/`""`), `Option` (`None`), reference-containers (defaulted to the empty container), and records/tuples built recursively from safe fields. A bare user `class` field zeroes to a nil pointer with no empty constructor, so omitting it would reintroduce the `nil` the language disclaims. Provide the field, or type it `Option<Class>` (which zero-defaults to `None`). Reported at the literal so the miss does not leak as a raw Go `SIGSEGV` on first use. |
| E0221 | E | Unused local binding | `name-resolution.md` §Local bindings | A `let`/`var` local that nothing references. Go rejects it ("declared and not used") with a raw go/types message; this diagnoses it in Aril coordinates. Reference it, or write `let _ = e` to discard intentionally. A reference through an assignment, a call, a field access, or a loop `invariant` counts as use; channel bindings are exempt (they participate in `select`/`spawn`/contracts). |
| E0222 | E | Duplicate local declaration | `name-resolution.md` §Local bindings | A `let`/`var` re-declares a name already bound in the **same** block. A nested `{ … }` block instead *shadows* the outer binding (its own Go scope). Rename, or move the second binding into a nested block if shadowing was intended. Replaces the raw Go `x redeclared in this block` leak. |
...
| H0002 | H | Bare `m[k]` on a class-valued map returns Go's zero value on a miss | `builtins.md` §Map; `lowering-go.md` §Container defaulting; AUDIT-3 T13 | A bare `m[k]` *read* whose value type has no safe default — a user `class`, or a record/tuple transitively containing one — yields a Go nil pointer on a missing key (the residual T13 hole: container-valued misses default to empty, scalars/`Option`/all-safe records zero safely, but a bare class pointer is a nil landmine that SIGSEGVs on first use). Read it as `m.get(k): Option<V>` and handle the `None` (guarding with `m.has(k)` still leaves the bare `m[k]` unsafe). Silent in an assignment target (`m[k] = v` is a store, never a miss) and sound-over-complete — mirrors codegen's container-defaulting exactly, and is suppressed when the key type already mismatches (E0201). |

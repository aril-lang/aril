...
| H0002 | H | Bare `m[k]` on a class-valued map returns Go's zero value on a miss | `builtins.md` §Map; `lowering-go.md` §Container defaulting; AUDIT-3 T13 | A bare `m[k]` *read* whose value type has no safe default — a user `class` — yields a Go nil pointer on a missing key (the residual T13 hole: container-valued misses default to empty, scalars/`Option`/records zero safely, but a bare class pointer is a nil landmine that SIGSEGVs on first use). Use `m.get(k): Option<V>` and handle the `None`, or guard with `m.has(k)`. Silent in an assignment target (`m[k] = v` is a store, never a miss) and sound-over-complete — fires only when V is statically a non-defaultable class. |

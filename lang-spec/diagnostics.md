...
| E0122 | E | Dependency version conflict | `manifest.md` §Fetch | Two modules in the resolved dependency graph pin the same dependency to different versions (v0.x is exact-pin — one module, one version; there is no minimal-version selection). The message names the dependency and both `source@version` pins. Reconcile the versions so the graph agrees on one. |

...
| E0214 | E | Type has no member `name` | `type-system.md` T-Field, T-Call (member access / method call over a receiver with a fully-known member set) | Access only a declared field or method of the receiver's type — a user class / interface / record, an opaque `extern` handle, a bound stdlib value-handle, or a builtin-generic type (`Map`/`Set`/`Stack`/`[]T`/`Option`/`Result`/channel, whose method set is fixed). A bare type parameter or an `Unknown`-typed receiver has no known member set, so it is not diagnosed here. |

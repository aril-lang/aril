...
func (g *gen) goTypeFromSema(t sema.Type) (string, bool) {
	...
	case *sema.Named:
		...
		// package spelling for an external one (D37). Without this the
		// raw Aril spelling `big.BigInt` would leak into the Go (undefined).
		if goType, ok := g.handleGoType(v.N); ok {
			return goType, true
		}
		return goIdent(v.N), true
	case *sema.Slice:
...
func (g *gen) bindSubPattern(sub ast.Pattern, valueExpr string) error {
	switch sp := sub.(type) {
	case *ast.IdentPat:
		// A `_` payload binding is a discard — bind nothing (`_ := x` is
		// illegal Go). Reached when `_` arrives as an IdentPat, not a
		// WildcardPat (desugaring.md §Catch; lowering-go.md §MatchIR).
		if sp.Name == "_" {
			return nil
		}
		g.writeIndent()
